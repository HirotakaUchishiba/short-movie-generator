...
ビジュアル要素 (背景・体勢・服装・場所・カメラ距離・動き)、**プラットフォーム UI 要素** (TikTok / Instagram / YouTube Shorts のハンドル名・いいね・コメント・ウォーターマーク)、**キャスティングの意味づけ** (主人公/上司/先輩のような役割ラベル) は **絶対に出力しないでください**。キャストや背景はユーザーが Stage 1 UI で speaker_to_ref / location_ref / character_selection / animation_style を per-scene に注入し、後段の合成フェーズで決定論的に展開されます。あなたのタスクはあくまで「中身」の抽象化です。プラットフォーム UI 要素は元動画固有の overlay で、生成動画には絶対に含めてはいけないため caption / text / 説明文のいずれにも反映しないでください。
  ...
  "scenes": [
    {
      ...
      "annotation": {
        "visual_intent_id": "(任意) intent catalog の id。入力 user content に \"# 利用可能な visual intent 集合\" が提供されている場合のみ出力。集合に無い id は禁止 (= 自動で null に降格される)。良い match が無いシーンは null を入れて confidence を低くする",
        "confidence": "(任意) visual_intent_id の確信度 0.0-1.0。0.7 未満は自動で null に降格される (= novel intent 候補としてオペレータに提示)。visual_intent_id を出した場合は必ず confidence も付ける",
...
# 絶対に出力しないフィールド (ビジュアル/環境系・Stage 1 UI で注入)
**以下のフィールドは出力スキーマに含めない**:
- scene 内: background_prompt / animation_prompt / animation_style / characters / character_refs / character_selection / location_ref / camera_distance / tags / lipsync
    ...
    *,
    ...
    Args:
        ...
        intent_catalog: Step 1 (analyze annotation 注入) で導入。
            ...
            出力された annotation は ``intent_resolver.normalize_scene_annotation``
            で id 存在 / confidence threshold check を通り、未知 id や低
            confidence は None に降格される (= novel intent escape hatch)。
    ...
    if intent_catalog:
        ...
        extra_blocks.append(
            ...
            "こと。良い match が無いシーンは null を入れて confidence を下げる "
    ...
    # 未知 id / 低 confidence は None に降格し、無効値は drop する。
    # 結果として scenes[i].annotation = {visual_intent_id?, duration_bucket?,
    # motion_intensity?} の形に整う (= 全フィールド drop なら annotation key
    # 自体を削除して旧スキーマ互換を保つ)。
    if intent_catalog:
        from analyze.intent_resolver import normalize_scene_annotation
        ...
        for s_idx, scene in enumerate(parsed.get("scenes") or []):
            ...
            if raw_ann is None:
                continue
            normalized = normalize_scene_annotation(raw_ann, intent_catalog)
            if normalized is None:
                scene.pop("annotation", None)
                logger.info(
                    "[annotation] scene %d: all fields demoted/dropped, "
                    "removed annotation key",
                    s_idx,
                )
            else:
