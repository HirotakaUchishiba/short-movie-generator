...
| **6. overlay** | 字幕焼き込み + caption   | FFmpeg/libass **OR** Remotion + Claude Haiku    | `output/reels_<TS>.mp4` (= pipeline raw) + `post_captions/<title>.md` |
...
│  compositor.py         (= ffmpeg backend)                  │
│  compositor_remotion.py (= Remotion backend)               │
│  clip_library.py       (= Layer 1 永続キャッシュ)          │
...
[git 追跡]
  ...
  config.py / config/part_registry/*.yaml
  ...
  "scenes": [
    {
      ...
      "scene_parts": {
        ...
        "stickers": [{ "id": "exclaim_red", "at": 1.2 }]
      }
    }
  ],
  "global_parts": {
    "filter_preset": { "id": "warm_cinematic" },
    "intro_card": { "id": "simple_intro" }
  }
}
```

- **identity / annotation** は #149-#151 で導入された clip_library cache 鍵 (詳細は §6.1)
...
## 6. Compositional Architecture (= Layer 1-3)

直近の最大の構造変化。動画素材を **3 つの層** に整理し、それぞれの責務とキャッシュ戦略を分離した。設計案は `2026-05-10_compositional-architecture.md`、現状図解は `2026-05-10_parts-and-composition-overview.md` を参照。

### 6.1 Layer 1: Clip Library (永続キャッシュ)

**目的**: 同条件のシーンを 2 回目以降は AI 課金 0 (= Imagen + Kling のスキップ)
...
### 6.2 Layer 2: Part Registry (code-defined + audio file)

**目的**: 重い AI 生成を介さず、軽量な「組立てパーツ」で表現を切替える

```
config/part_registry/<category>.yaml   ← SSOT (= id 定義)
    ↕ drift test で id 集合一致を強制 (= part_registry_yaml_drift.test.ts)
frontend/remotion/parts/<category>/    ← React 実装
```

利用可能な 8 code-defined カテゴリ + audio file:

| 階層       | category          | 利用可能 id                                                        |
| ---------- | ----------------- | ------------------------------------------------------------------ |
| **scene**  | `subtitle_styles` | `minimal` / `fade_in` / `karaoke_bold`                             |
| **scene**  | `stickers`        | `exclaim_red` / `question_mark` / `sparkle` / `thumbs_up` / `fire` |
| **scene**  | `camera_moves`    | `none` / `subtle_zoom_in` / `ken_burns` / `dolly_pull_back`        |
| **scene**  | `lower_thirds`    | `name_banner` / `role_caption` / `quote_box`                       |
| **scene**  | `transitions`     | `cut` / `dip_to_black` / `dip_to_white` / `fade_quick`             |
| **scene**  | `frame_layouts`   | `full` / `letterbox_top_bottom` / `centered_with_blur`             |
| **scene**  | sfx (= file)      | `assets/sfx/*` (= path 直接指定)                                   |
| **global** | `filter_presets`  | `none` / `warm_cinematic` / `cool_blue` / `monochrome` / `vintage` |
| **global** | `title_cards`     | `simple_intro` / `subscribe_outro` / `section_break`               |
| **global** | bgm (= file)      | `assets/bgm/*` (= path 直接指定 + ducking_curve 任意)              |

**重要不変条件**: パーツの差し替え / 新規追加で **AI 課金は発生しない**。再 render するだけで反映される。

### 6.3 Layer 3: Composition Engine (= Remotion)

**目的**: scene 動画 (= AI 生成、Stage 1-5) と Layer 2 パーツを React/Remotion で組立て、最終 mp4 を吐く

#### Composition tree

```
ScreenplayBase (= AbsoluteFill 黒背景)
├── FilterWrapper (= global_parts.filter_preset)
│   └── SceneStack
│       └── 各 SceneSequence:
│           ├── FrameLayout wrap (= scene_parts.frame_layout)
│           │   └── CameraMove wrap (= scene_parts.camera_move)
│           │       └── <OffthreadVideo> scene_<S>.mp4
│           ├── 字幕 <Sequence> × N chunks (= scene_parts.subtitle_style)
│           ├── sticker <Sequence> × N (= scene_parts.stickers[])
│           ├── lower_third <Sequence> (= scene_parts.lower_third)
│           ├── transition_in / transition_out (= 冒頭 / 末尾 N frame)
│           └── sfx <Audio> × N (= scene_parts.sfx[])
├── <Audio> bgm (= global_parts.bgm)
├── intro_card <Sequence> (= 冒頭)
└── outro_card <Sequence> (= 末尾)
```

**wrapping 順序の不変条件** (= 内側 → 外側): `<OffthreadVideo>` → `CameraMove` → `FrameLayout`。overlays (字幕 / sticker / lower_third / transition / sfx) は wrapping されず `<AbsoluteFill>` の上に並列配置 (= 動画 transform の影響を受けない)。

#### Platform variant fan-out

```
Stage 1-5 (= AI 課金、1 回のみ) → scene_<S>.mp4 群
                                         ↓
            compose_video_remotion(template="youtube"|"instagram"|"tiktok"|"base")
                                         ↓
                3 platform 分の reels_<TS>__<platform>.mp4 が AI 課金 0 で吐ける
```
...
| `instagram` | subtitle_style 全 scene で `karaoke_bold` 強制                                     |
| `tiktok`    | `karaoke_bold` 強制 + 字幕 y_from_bottom = 640 + outro_card = `section_break` 1.0s |

#### Backend dispatch

| `OVERLAY_BACKEND` | backend                  | 動作                                        |
| ----------------- | ------------------------ | ------------------------------------------- |
| `ffmpeg` (= 既定) | `compositor.py`          | 従来通り (= drawtext 焼き込み)              |
| `remotion`        | `compositor_remotion.py` | Remotion CLI で React composition を render |

**重要不変条件**: timing 計算 (= subtitle chunk の絶対秒解決) は **Python 側 SSOT** (`compositor.py` / `compositor_remotion.build_render_plan`) で一括実行。Remotion は受け取った数値を信用するだけで、勝手に演出を足さない。

詳細: `docs/plannings/2026-05-10_parts-and-composition-overview.md` (= mermaid 図解)、`docs/plannings/2026-05-10_compositional-architecture.md` (= 設計 authoritative)
...
- `screenplay_validator.py` で検証 (= ASCII `,`/`.` reject、必須フィールド確認、part_registry yaml との id 整合性)
...
- ElevenLabs `eleven_v3` で **screenplay 全体を 1 API call** で生成 (= `generate_screenplay_tts_one_shot`)
- 連続音声で char-level timestamps を取得 → silence-detect で line 境界を snap → 自然な抑揚と pacing を実現
- `voice_id` / `stability` / `similarity_boost` / `style` / `speed` は **screenplay-wide で 1 セット** (= per-line 切替不可)
...
- 出力: `tmp/tts_<S>_<L>.mp3` 群 + `tts_meta.json` (= duration / start / end の派生値)
...
## 8. Composition Engine 詳細 (Stage 6)
...
詳細: `CLAUDE.md` 「オーバーレイ」「Stage 6 backend dispatch」
...
|                  | `OVERLAY_BACKEND`                                               | 任意              | Stage 6 backend (= `ffmpeg` (= 既定) / `remotion`)          |
|                  | `CLIP_LIBRARY_ENABLED`                                          | 任意              | Layer 1 永続キャッシュ (= 1 で wire)                        |
...
1. **AI 課金が増える方向の変更は禁止**: 字幕修正で動画再生成しない。Layer 2 (= 軽いパーツ) の編集は再 render だけで済む。参照画像差替え (= identity hash 変化) のみが Stage 3+4 cold path を引き起こす
2. **Layer 1 cache 識別性**: identity 4 軸 `(character_refs, location_ref, start_emotion, camera_distance)` が一致する 2 つの scene は **必ず同じ entry pool に hit する**
3. **Layer 2 は code 内のみ**: subtitle/sticker 等の見た目変更は **再 render だけ** で済む。AI 課金は発生しない
4. **timing 計算は Python SSOT**: subtitle chunk の絶対秒は `compositor.py` / `compositor_remotion.py` で一括解決。Remotion はその数値を信用するだけ
5. **drift test が yaml ↔ component を強制一致**: `part_registry_yaml_drift.test.ts` が壊れたら CI fail
6. **decision (= 何のパーツを使うか) は screenplay**: Composition (= Remotion) は受け取った plan を忠実に描画するだけで、勝手に演出を足さない
7. **screenplay の編集は project snapshot のみ**: template (`screenplays/<name>.json`) は project 作成後に触らない (= 進行中 project 不変の担保)
8. **段階的ゲートを破らない**: 承認なしに次 stage に進む経路を作らない
9. **指示の範囲を超えない**: 「字幕を修正して」と言われたら字幕だけ。台本テキスト・シーン構成・動画は触らない

---

## 14. 直近の主な変化 (= 2026-05-10 時点)

1. **Compositional Architecture が main 着地**: Layer 1-3 すべて wire 完了 (= `2026-05-10_parts-and-composition-overview.md` の構造どおり)
2. **Layer 1 clip_library が動く**: `CLIP_LIBRARY_ENABLED=1` で `staged_pipeline.run_bg` から `satisfy_scenes_from_library` が動き、`run_scene` 直前で `register_cold_path_clips` が動く。同条件 scene の 2 回目以降 Imagen + Kling 課金 0
3. **analyze pipeline 設計準拠完了** (PR #149-#151): identity 派生 + annotation 注入 + error_code 統一。これで Layer 1 cache hit が **構造的に発動する** 状態に
4. **Remotion backend 選択可能**: `OVERLAY_BACKEND=remotion` で platform 別 variant が AI 課金 0 で吐ける
5. **抽象台本 + compose 合成**: 参考動画から作る台本もビジュアル決定を分離 (= 構成・セリフ・感情だけ抽出、ビジュアルは scene 個別 + 話者マッピングで後段注入)
...
| Layer 1 clip_library        | ✅ wire 完了 (= `CLIP_LIBRARY_ENABLED=1`)                           |
| Layer 2 part_registry       | ✅ 8 categories + bgm/sfx 全 wire。yaml ↔ component drift test 有り |
| Layer 3 Composition         | ✅ ScreenplayBase / Youtube / Instagram / TikTok 完成               |
...
- `2026-05-10_compositional-architecture.md` — Layer 1-3 設計 (authoritative)
- `2026-05-10_parts-and-composition-overview.md` — Layer 1-3 現状図解 (mermaid)
...
- `2026-05-10_architecture-mismatch-audit.md` — Compositional Architecture 設計 ↔ 実装の差分監査 + 修正履歴
- `2026-05-10_full-pipeline-conformance-audit.md` — Stage 1-8 + analyze + analytics 横断監査
- `2026-05-10_analytics-pdca-gap-and-remediation.md` — analytics PDCA 3 phase plan (= PR #153 / #158 / #161 で完了)
- `2026-05-10_clip-library-architecture.md` — Layer 1 cache の identity match + annotation rank 設計
- `2026-05-10_remotion-integration-design.md` — Remotion CLI 統合 + render_plan 設計
...
- `2026-05-08_phase-0-implementation.md` 〜 `2026-05-08_phase-4-implementation.md` — 抽象台本 / Compositional Architecture 立ち上げ Phase 0-4 実装記録
