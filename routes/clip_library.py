...
def project_clip_status(ts: str):
    ...
    try:
        ...
        abort(500, f"screenplay.json 読込失敗: {e}")
    enabled = bool(getattr(config, "CLIP_LIBRARY_ENABLED", False))

    out: list[dict[str, Any]] = []
    for s_idx, scene in enumerate(sp.get("scenes") or []):
        has_id = clip_library._scene_has_identity(scene)
        record: dict[str, Any] = {
            "scene_idx": s_idx,
            "has_identity": has_id,
            "satisfied": False,
        ...
        if has_id:
            try:
                ...
                if pool:
                    # variant 選択は決定論なので、UI には top-1 を表示
                    entry = clip_library.select_variant(
                        pool, os.path.basename(ts), s_idx
                    )
