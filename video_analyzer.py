...
ビジュアル要素 (背景・体勢・服装・場所・カメラ距離・動き) と **キャスティングの意味づけ (主人公/上司/先輩のような役割ラベル)** は **絶対に出力しないでください**。キャストや背景はユーザーが Stage 1 UI で speaker_to_ref / location_ref / character_selection / animation_style を per-scene に注入し、後段の合成フェーズで決定論的に展開されます。あなたのタスクはあくまで「中身」の抽象化です。
