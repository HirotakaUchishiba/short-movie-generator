...
def api_set_canonical_final(ts, filename):
    ...
    if not _FILENAME_RE.match(filename):
        return api_error("FINAL_INVALID_FILENAME", "invalid filename", 400, filename=filename)
    from final_import import core as fi
    try:
        v = fi.set_canonical_final(ts_path(ts), filename)
    except ValueError as e:
        return api_error("FINAL_NOT_FOUND", str(e), 404, filename=filename)
...
def api_publish(ts):
    ...
    if platform not in ("youtube", "instagram", "tiktok"):
        return api_error("PUBLISH_INVALID_PLATFORM", f"invalid platform: {platform}", 400, platform=platform)
    privacy = data.get("privacy", "private")
    if privacy not in ("private", "unlisted", "public"):
        return api_error("PUBLISH_INVALID_PRIVACY", f"invalid privacy: {privacy}", 400, privacy=privacy)
