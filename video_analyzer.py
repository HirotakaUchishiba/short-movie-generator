...
与えられた動画フレーム画像・音声文字起こし・音響特徴・ショット境界・話者プロファイル・BGM情報から、以下のスキーマに厳密に従うJSON台本を生成してください。

# 出力スキーマ（必ずこの構造で返す）
{
  "caption": "SNS投稿用キャプション本文（ハッシュタグ含む、\\nで改行可）",
  "title_overlay": "動画上部固定の黄色帯タイトル（\\nで改行可、省略可）",
  ...
  "scenes": [
    {
      "time": "画面下の時刻表示（例 \"8:50\"、省略可）",
      "label": "時刻の下のシーンラベル（例 \"起床\"、省略可）",
      ...
      "characters": [
        {
          ...
          "ref": "characters/<name>.png のキー（無ければ省略）",
          "outfit": "服装の自然言語記述"
        }
      ],
      "wardrobe": {
        "identifier": "wardrobe_continuity に書いたID",
        "top": "上着の説明",
        "bottom": "ボトムの説明",
        "accessories": "メガネ・髪型・小物",
        "hair": "髪型"
      },
      "facial_expression": "シーン全体での主役の表情（例 \"細目で寝起き顔→驚いて目を見開く\"）",
      "hand_gesture": "シーン全体での手の動き（例 \"スマホを見せる\" \"頭を抱える\"）",
      "lines": [
        {
          "speaker": "characters[].name と一致",
          ...
          "pause_before": このlineの直前に挿入する無音秒数 (任意),
          "breath_before": true|false (吸気音の有無)
        }
      ]
    }
  ],
  "_analysis": {
    "source_video": "入力動画のパス",
...
- background_prompt は必ず日本語（被写体）+ 英語（cinematic lighting, shallow depth of field 等）を混ぜる
- animation_prompt は英語で、そのシーンの動き全体を1文で表現
- emotion は単語1つ。複合感情は主要なものを選ぶ
- 各シーンは1本のKlingクリップで生成される。ショット境界情報は scene 分割の参考にする
- characters: 1人のみのシーンでも明示的に1要素配列として埋める
...
- wardrobe: 主役の服装を必ず埋め、wardrobe_continuity の identifier と紐付ける（同じ服装の場合は同一ID）
...
ショット境界・無音区間・話者プロファイル・BGM存在判定が text入力として与えられる。これらを統合的に判断。
"""
...
def _format_transcript_block(transcript: dict, phrase_features: list[dict]) -> str:
    ...
    for i, seg in enumerate(transcript.get("segments", [])):
        ...
        feat_str = (f" pitch={feat.get('pitch_hz_mean', 0):.0f}Hz "
                    f"trend={feat.get('pitch_trend', 'flat')} "
...
def build_screenplay(
    *,
    ...
    shot_boundaries: list[dict] | None = None,
    pauses: list[dict] | None = None,
    voice_profile_info: dict | None = None,
    ...
    if shot_boundaries:
        shots_str = "\n".join(
            f"- shot[{i}]: t={s['start']:.2f}-{s['end']:.2f} ({s['duration']:.2f}s)"
            for i, s in enumerate(shot_boundaries)
        )
        extra_blocks.append(f"# 検出されたショット境界\n{shots_str}")
    if pauses:
        pauses_str = "\n".join(
            f"- pause: t={p['start']:.2f}-{p['end']:.2f} ({p['duration']:.2f}s)"
            for p in pauses
        )
        extra_blocks.append(f"# 検出された無音区間（pause/breath候補）\n{pauses_str}")
    if voice_profile_info:
        vp_str = (
            f"- 中央ピッチ: {voice_profile_info.get('pitch_hz_median', 0):.0f}Hz "
            f"(p10={voice_profile_info.get('pitch_hz_p10', 0):.0f} "
            f"p90={voice_profile_info.get('pitch_hz_p90', 0):.0f})\n"
            f"- 推定性別: {voice_profile_info.get('estimated_gender', '?')}\n"
            f"- 推定年代: {voice_profile_info.get('estimated_age_range', '?')}"
        )
        extra_blocks.append(f"# 話者プロファイル\n{vp_str}")
    if bgm_info:
        extra_blocks.append(
            f"# BGM存在判定\n"
            f"- present: {bgm_info.get('present')}\n"
            f"- confidence: {bgm_info.get('confidence', 0):.2f}\n"
            f"- ratio: {bgm_info.get('ratio', 0):.2f}"
        )
    if known_furigana:
        sample = "\n".join(
            f'  "{k}": "{v}"'
    ...
    parsed.setdefault("_analysis", {})
    parsed["_analysis"].update({
        "source_video": source_video_path,
        "analyzer_model": ANALYZER_MODEL,
    })

    if usage_input is not None:
        parsed["_analysis"]["input_tokens"] = usage_input
    if usage_output is not None:
        parsed["_analysis"]["output_tokens"] = usage_output
