...
参考動画から台本 JSON を生成する際、Claude Vision の出力を「**元動画クローン**」ではなく「**構成・セリフ・感情・ロケ選定だけ抽出した抽象台本**」に変える。analyze pipeline は構成・セリフ・感情に加えて `location_ref` / `camera_distance` を `locations/` カタログから自動選定する (= 詳細プロンプトやキャラ casting は産出しない)。これにより:
...
| 3   | `scenes[].animation_style`     | scene | "standard"                        |
| 4   | `scenes[].character_selection` | scene | 未指定 (= speakers から自動推論)  |
...
analyze が必ず産出する (= 欠落は fail-fast)。`annotation` は best-effort で、
validator は annotation 不在を許容する (= 旧 abstract 後方互換)。
