...
### 4.2 Provider 切替 (lipsync の例)

複数の選択肢がある外部 API は **config で切替可能** にし、共通 interface に揃える。

| 抽象            | 具象                                              | 切替               |
| --------------- | ------------------------------------------------- | ------------------ |
| `LipsyncClient` | `SyncsoClient` / `DomoaiClient` / `FalSyncClient` | `LIPSYNC_PROVIDER` |

将来 TTS / BG にも複数 provider を入れる場合は同パターン。

### 4.3 Idempotency と中断再開
