#!/usr/bin/env python3
"""Stage 4 (Kling 動画) のグローバルキャッシュを一覧 / 統計表示する CLI。

例:
    python3 scripts/inspect_kling_cache.py
    python3 scripts/inspect_kling_cache.py --sort hits --top 30
    python3 scripts/inspect_kling_cache.py --location home_office
"""
...
def main() -> int:
    ...
    print(f"{'key':<18} {'size':>8} {'hits':>5} {'kdur':>4} {'orig':>6} "
    ...
    for e in entries[:args.top]:
        ...
        print(f"{e['key']:<18} {_fmt_bytes(e['size_bytes']):>8} "
              f"{e['hit_count']:>5} {str(e['kling_duration'] or '-'):>4} "
              f"{(f'{e['original_audio_duration']:.1f}' if e['original_audio_duration'] else '-'):>6} "
