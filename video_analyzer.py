...
- 確信が持てない speaker は speaker_to_ref から省略してよい (= 人間が Stage 1 UI で対応付ける)
        ...
        if cleaned_s2r and location_catalog:
            ...
            for entry in location_catalog:
                ...
                if isinstance(loc_id, str) and isinstance(recs, list):
                    loc_to_wardrobes[loc_id] = [
                        r for r in recs if isinstance(r, str)
                    ]
            ...
            for entry in character_catalog:
                ...
                if isinstance(base_id, str) and isinstance(refs, list):
                    base_to_refs[base_id] = [
                        r for r in refs if isinstance(r, str)
                    ]
            # speaker → 登場シーンの location_ref を line 単位で集計 (= 多くの
            # 台詞を持つ location が dominant)
            ...
            for sc in parsed.get("scenes") or []:
                ...
                for line in sc.get("lines") or []:
                    ...
                    if isinstance(spk, str):
                        speaker_to_locs.setdefault(spk, []).append(loc)
            ...
            for spk, ref in cleaned_s2r.items():
                ...
                # 同数のときは最初に登場した location を選ぶ (= 決定論性)
                dominant = next(
                    lc for lc in locs if counts[lc] == max_count
                )
