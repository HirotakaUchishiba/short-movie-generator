...
def _normalize_action_and_intent(parsed: dict) -> None:
    ...
    実体は ``screenplay_validator.normalize_abstract_annotations`` に共通化済み
    (= PUT abstract 保存経路でも同じ正規化を使い、emotion 編集で不整合になった
    annotation を drop するため)。
    """
    from screenplay_validator import normalize_abstract_annotations

    normalize_abstract_annotations(parsed)
