...
| パターン                                     | 代替                                                                              |
| -------------------------------------------- | --------------------------------------------------------------------------------- |
| マジックナンバー                             | 定数化 + 名前で意味を表す (`UPLOAD_STATE_TTL_SEC = 24 * 3600`)                    |
...
| 「歴史的記録」型の半端注釈を残さない  | 旧情報は git history に任せて削除する。「以下は歴史的記録」「旧 X は撤廃済み」型の注釈で旧記述を放置せず、本文を新スキーマに書き換える (= 過去の設計案は専用セクションに集約) |
