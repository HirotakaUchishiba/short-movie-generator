...
| ステータス   | implementation plan (= ベース設計の詳細化、Phase A〜E 着手前)         |
...
def create_job(
    ...
    *,
    ...
    更新する。None なら旧経路 (= standalone analyze、Phase E で削除予定)。
    """
    job_id = _new_job_id()
...
interface Props {
   ...
   * standalone モード (= 旧 /analyze ページ) では「プロジェクト作成」ボタンを表示。
   * Phase E で旧モードを削除する。
   */
...
- これにより「Phase E で旧 endpoint を消すとロジック重複が消える」問題が起こらない
