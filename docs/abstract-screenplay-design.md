...
| 6   | `speaker_profiles`                     | root  | 匿名 speaker ごとの gender / age_range / description。フレーム + 音響から検出 (= 話者マッピング UI のヒント表示用)                                                                                                                                                 |
| 7   | `featured_characters`                  | root  | character catalog が提供されれば Claude が catalog の base から順番に割り当てて提案 (= 元動画の登場人物に寄せない)。catalog 外 ref は後処理で drop                                                                                                                 |
| 8   | `speaker_to_ref`                       | root  | 同上。speaker_N → resolved id の提案。**speaker_profiles を持つ全 speaker に必ず 1 ref を割り当てる** (= Claude が省略した分は post-process の `_fill_unmapped_speakers` が alphabetical 順 + distinct rule で deterministic に補完。人間が Stage 1 UI で訂正可能) |

#### casting 後処理ルール (= post-processing で決定論的に適用)

Claude の提案には 3 つの判定ロジックが後処理で被さる。すべて graceful (= 適用不能なら何もしない)。**いずれも参考動画の登場人物に寄せるロジックは含まない** (= 2026-05-17 方針変更、`docs/plannings/2026-05-17_decouple-casting-from-reference.md` 参照)。
...
- **Phase D (unmapped fallback)**: 上記 2 ルール後に **`speaker_profiles` を持つ全 speaker が `speaker_to_ref` に登場するか** を確認し、欠落 speaker を `_fill_unmapped_speakers()` が catalog の base を alphabetical 順に未使用から割当てる (= distinct rule 優先、枯渇時は alphabetical 先頭から再利用)。wardrobe は dominant location に合わせる。Stage 1 UI で「未選択」状態を構造的に発生させない
