...
本ドキュメントは short_movie_generator のドメイン用語と実装上の名前の対応表。会話・ドキュメント・コードで同じ概念を別名で呼ばないようにするための辞書。
...
| 抽象台本          | analyze pipeline 出力            | ビジュアル要素を持たない台本。`line.speaker` には **resolved id 直書き** (= 2026-05-17 `speaker_to_ref` mapping 撤廃) |
| 完全台本          | compose 後                       | 抽象台本 + scene 個別フィールド (= identity / annotation / location_ref / camera_distance) を compose した最終形      |
| 言い換え          | `gemini_dialogue_rewriter`       | analyze 後段で Gemini 2.5 Pro が `line.text` + `caption` を翻案権配慮で書き換える phase。同義 / 同感情を保つ          |
...
| 推奨衣装           | `locations/<id>.json:recommended_wardrobes` | analyze の casting で dominant location の本リストに合う wardrobe バリアントへ swap される (= 2026-05-16) |
...
| compose           | `analyze/compose.py`       | 抽象台本 + scene 個別 visual fields (= identity 派生) を組んで完全台本にする                                            |
| dialogue rewrite  | `gemini_dialogue_rewriter` | analyze 後段で Gemini 2.5 Pro が line.text + caption を翻案権配慮で書き換える (= 2026-05-17)                            |
| 再生成            | UI の各カードのボタン      | 該当 stage の単位アーティファクトを再実行                                                                               |
| TTS               | Stage 2                    | ElevenLabs eleven_v3 で生成。unique speaker 数で経路分岐 (= 単一話者は 1-shot / 2+ は per-character 並列)               |
| per-character TTS | `per_character_tts.py`     | 2+ speakers のとき N voice_id 並列で同 screenplay 全文を生成し、各 line を該当 speaker から切出 + concat (= 2026-05-17) |
