...
> ℹ️ **2026-05-10 audit 後**: Phase 1 (clip_library) は wire 完了
> (= `CLIP_LIBRARY_ENABLED=1` で satisfy + register が Stage 3/5 に hook、PR #134)。
> Phase 6 (intent_resolver) は依然 skeleton (= LLM 統合は次セッション)。
> 詳細は `docs/plannings/2026-05-10_architecture-mismatch-audit.md` §5 修正履歴を参照。

| Phase | 内容                                                               | status                                                                                   |
| ----- | ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------- |
| 0     | Remotion セットアップ + HelloWorld                                 | ✅                                                                                       |
| 1     | clip_library (= identity/annotation/provenance)                    | ✅ wired (= `CLIP_LIBRARY_ENABLED=1` で opt-in、satisfy + register が Stage 3/5 に hook) |
...
- **Phase 1 (clip_library)** — ✅ wire 完了 (PR #134)。
  `CLIP_LIBRARY_ENABLED=1` で `staged_pipeline.run_bg` が `satisfy_scenes_from_library`
  を呼び、identity 一致 scene の bg + kling を temp/ にコピーして Stage 3/4 を
  短絡する。`run_scene` 直前で cold path 生成物を `register_cold_path_clips` で
  library に追加し cache が育つ。default は OFF (= 既存挙動完全維持)。
...
- **新フィールド `_override_*`** — ✅ wire 完了 (PR #133)。`scene_gen` の
  `_build_background_prompt` / `_get_animation_prompt` で early return される。
- **新フィールド `scene_parts` / `global_parts`** — Stage 6 (Remotion) のみ消費、
  Stage 1-5 (AI 生成) では現状無視される (= mismatch 1-2、別セッションで拡張)。
  validator は part_registry yaml と id 整合性を ✅ 検証する (PR #132)。
