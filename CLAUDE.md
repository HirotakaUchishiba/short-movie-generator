...
### Clip Library の cache key — `visual_intents`
...
の cache lookup key (= identity + intent) として使われる。validator は
`config/part_registry/visual_intents.yaml` の id 集合と突合し、未定義 id は
fast-fail する (= drift 監査の対象)。
