...
自由記述のビジュアル要素 (背景プロンプト・体勢・服装・動き)、**プラットフォーム UI 要素** (TikTok / Instagram / YouTube Shorts のハンドル名・いいね・コメント・ウォーターマーク)、**キャスティングの意味づけ** (主人公/上司/先輩のような役割ラベル) は **絶対に出力しないでください**。character_selection / animation_style はユーザーが Stage 1 UI で per-scene に注入します。あなたのタスクは「中身」の抽象化に加えて、(1) "# 利用可能な location 集合" が提供されていれば各 scene の location_ref / camera_distance を選定し、(2) 複数人物の動画では各 speaker の speaker_profiles を検出し、さらに "# 利用可能な character 集合" が提供されていれば featured_characters / speaker_to_ref を提案することです (= 下記ルール参照)。提案系フィールドはユーザーが Stage 1 UI で訂正できます。プラットフォーム UI 要素は元動画固有の overlay で、生成動画には絶対に含めてはいけないため caption / text / 説明文のいずれにも反映しないでください。
...
# 出力スキーマ (これだけが許容)
{
  ...
  "speaker_profiles": {
    "speaker_1": {
      ...
      "age_range": "(任意) 10s|20s|30s|40s|50s+ など",
      "description": "(任意) 話し方・雰囲気の自然言語 1 行 (例: \"明るく早口でリアクション大きめ\")"
    }
  },
  "featured_characters": ["(任意) \"# 利用可能な character 集合\" が提供されている場合のみ出力。動画に登場させる resolved id の配列"],
  "speaker_to_ref": {"speaker_1": "(任意) \"# 利用可能な character 集合\" が提供されている場合のみ出力。speaker_N → resolved id"},
  "scenes": [
    {
      ...
      "lines": [
        {
          "speaker": "speaker_1 / speaker_2 / ... の連番ID (匿名)。speaker_profiles を出力する場合は **全 line に必ず付ける**。speaker_profiles を出力しない単一人物動画でのみ省略可",
...
# speaker_profiles / casting 出力ルール
複数人物の動画 (= speaker タグを振った動画) では、各匿名 speaker について root の speaker_profiles を必ず出力する:
- gender / age_range / description をフレーム画像 + 音響特徴から推定する。すべて optional だが、わかる範囲で埋める
- description は話し方・雰囲気を 1 行で (例: "落ち着いた低い声でゆっくり")
- 単一人物の動画 (= 全 line を同一人物が話している) では speaker_profiles と各 line の speaker タグの **両方を完全に省略する**
- **重要な整合性ルール**: speaker_profiles を 1 つでも出力するならば、**全 line.speaker に必ず speaker_N タグを振る** (= profiles を出すなら line タグも必須、片方だけは禁止)。これは Stage 1 UI が「speaker_profiles で出した speaker は line.speaker で実際に発話されている」という前提で動くため

入力 user content に "# 利用可能な character 集合" セクションが含まれている場合、加えて casting を提案する:
- speaker_to_ref: speaker_N → resolved id のマッピング。catalog 内の base から **順番に 1 つずつ** 割り当てる (= 参考動画の登場人物に寄せる必要は無い、台本作成時に Stage 1 UI で人間が自由に選び直す前提)。catalog の base 順 (= alphabetical) で speaker_1, speaker_2, ... に振ればよい
- **distinct character rule**: 異なる speaker には異なる **base** character を割り当てる (= 同じ base を 2 つの speaker にマッピングしない。後処理で 2 件目以降は drop される)
...
- featured_characters: 動画に登場させる resolved id の配列。speaker_to_ref の値を含む和集合
- **catalog の refs に無い id は出力しない** (= パイプラインで drop される)
- **speaker_profiles を持つ全 speaker に対して speaker_to_ref を必ず 1 つ提案する** (= 空欄は許可しない)。catalog 枯渇等で本当に提案不能なときのみ post-process が deterministic fallback で補完する
character 集合が提供されていない場合は featured_characters / speaker_to_ref を完全に省略する (= 旧挙動互換)。speaker_profiles は character 集合の有無に関わらず出力する (= Stage 1 UI のヒント表示用)。
...
# ─── casting post-process helpers ──────────────────────────────────
# build_screenplay の post-process で再利用する deterministic helper 群。
# テスト容易性のため module-level に出している。


def _compute_loc_to_wardrobes(
    location_catalog: list[dict],
) -> dict[str, list[str]]:
    """location id → recommended_wardrobes list の辞書を作る。"""
    ...
    for entry in location_catalog:
        if not isinstance(entry, dict):
            continue
        loc_id = entry.get("id")
        recs = entry.get("recommended_wardrobes") or []
        if isinstance(loc_id, str) and isinstance(recs, list):
            out[loc_id] = [r for r in recs if isinstance(r, str)]
    return out


def _compute_base_to_refs(
    character_catalog: list[dict],
) -> dict[str, list[str]]:
    """base id → resolved refs list の辞書を作る。"""
    ...
    for entry in character_catalog:
        ...
        if isinstance(base_id, str) and isinstance(refs, list):
            out[base_id] = [r for r in refs if isinstance(r, str)]
    return out


def _compute_speaker_to_locs(parsed: dict) -> dict[str, list[str]]:
    """parsed.scenes から speaker → 登場 location_ref list を集計。

    line 単位で集計 (= 多くの台詞を持つ location が dominant)。
    """
    ...
    for sc in parsed.get("scenes") or []:
        ...
        for line in sc.get("lines") or []:
            ...
            if isinstance(spk, str):
                out.setdefault(spk, []).append(loc)
    return out


def _dominant_location(
    spk: str, speaker_to_locs: dict[str, list[str]]
) -> str | None:
    """speaker の dominant location (= 最頻、同数なら最初の登場)。"""

    locs = speaker_to_locs.get(spk) or []
    if not locs:
        return None
    counts: dict[str, int] = {}
    for lc in locs:
    ...
    return next(lc for lc in locs if counts[lc] == max_count)


def _pick_wardrobe_aware_ref(
    *,
    base: str,
    spk: str,
    base_to_refs: dict[str, list[str]],
    loc_to_wardrobes: dict[str, list[str]],
    speaker_to_locs: dict[str, list[str]],
) -> str | None:
    """base に対し dominant location の recommended_wardrobes を加味した
    resolved id を選ぶ。

    優先順位:
      1. dominant location の recommended_wardrobes に合う ref
      2. base の最初の ref (= alphabetical な base_to_refs[base][0])
      3. None (= base に ref が無い)
    """

    refs = base_to_refs.get(base) or []
    if not refs:
        return None
    dominant = _dominant_location(spk, speaker_to_locs)
    if dominant:
        recs = loc_to_wardrobes.get(dominant) or []
        for w in recs:
            cand = f"{base}__{w}"
            if cand in refs:
                return cand
...
def _fill_unmapped_speakers(
    *,
    speaker_profiles: dict,
    cleaned_s2r: dict[str, str],
    character_catalog: list[dict],
    base_to_refs: dict[str, list[str]],
    loc_to_wardrobes: dict[str, list[str]],
    speaker_to_locs: dict[str, list[str]],
) -> dict[str, str]:
    """Claude が埋め残した speaker を deterministic に補完する。

    appearance に依存しない単純割当 (= 2026-05-17 方針変更
    `docs/plannings/2026-05-17_decouple-casting-from-reference.md` 参照):
      1. unmapped = speaker_profiles のキー - cleaned_s2r のキー
      2. 各 unmapped speaker を sorted 順に処理 (= 決定論性)
      3. catalog の base を alphabetical で走査して未使用 base を選ぶ
         (= distinct character rule)
      4. 候補枯渇時は alphabetical 先頭から再利用 (= distinct を緩める)
      5. 選ばれた base から wardrobe-aware ref を取り cleaned_s2r に追加

    元動画の登場人物との一致は意図的に取らない (= 参考動画と生成動画の
    登場人物は別物。人間が Stage 1 UI で自由に選び直す前提)。
    """

    out = dict(cleaned_s2r)
    used_bases: set[str] = {
        ref.split("__", 1)[0] for ref in out.values()
    }
    unmapped = sorted(
        spk for spk in speaker_profiles
        if isinstance(spk, str) and spk not in out
    )
    if not unmapped:
        return out

    # catalog から base id を alphabetical 順で 1 度だけ抽出
    sorted_bases: list[str] = sorted({
        entry.get("id") for entry in character_catalog
    ...
    for spk in unmapped:
        # Step A: distinct rule で空き base を探す (= alphabetical 先頭)
        chosen_base: str | None = None
        for base_id in sorted_bases:
            if base_id not in used_bases:
                chosen_base = base_id
                break
        # Step B: 枯渇したら alphabetical 先頭から再利用 (distinct を緩める)
        if chosen_base is None:
            chosen_base = sorted_bases[0]
        ref = _pick_wardrobe_aware_ref(
            base=chosen_base,
            spk=spk,
            base_to_refs=base_to_refs,
            loc_to_wardrobes=loc_to_wardrobes,
            speaker_to_locs=speaker_to_locs,
        )
        if ref is None:
            continue
        logger.info(
            "[casting] %s: unmapped speaker を順番割当で '%s' に補完", spk, ref,
        )
        out[spk] = ref
        used_bases.add(chosen_base)
    return out
...
def build_screenplay(
    *,
    ...
    if character_catalog:
        ...
        extra_blocks.append(
            "# 利用可能な character 集合\n"
            "各匿名 speaker の speaker_profiles をこの集合の appearance と "
            "突合し、featured_characters (= 登場させる resolved id の配列) と "
            "speaker_to_ref (= speaker_N → resolved id) を提案すること。"
            "id は各 entry の refs から選ぶ (= refs に無い id は drop される)。"
            "appearance が無いキャラも候補にしてよい。確信が持てない speaker は "
            "speaker_to_ref から省略してよい (= 人間が後で対応付ける)。\n\n"
            f"```json\n{character_json}\n```"
        )
    ...
    # ── speaker_profiles と line.speaker の整合性担保 ──
    # SYSTEM_PROMPT は「speaker_profiles を出すなら全 line に speaker_N を
    # 必ず振る」と指示しているが、Claude の判断が揺らぐと profiles だけ
    # populate されて line.speaker が全 null のケースが起きる
    # (= 2026-05-17 PR #205 で報告された SpeakerMappingSection 非表示 bug
    # の根本原因)。defensive に整合性を回復する:
    #   - speaker_profiles が 1 speaker のみ → 全 line に当該 speaker_N を backfill
    #   - speaker_profiles が 2+ speakers → ambiguous なので warn のみ (= 復旧不可)
    sp_profiles = parsed.get("speaker_profiles")
    if isinstance(sp_profiles, dict) and sp_profiles:
        valid_speaker_ids = sorted(
            k for k in sp_profiles
            if isinstance(k, str) and k.startswith("speaker_")
        )
        if valid_speaker_ids:
            ...
            if total_lines > 0 and lines_with_speaker == 0:
                if len(valid_speaker_ids) == 1:
                    single = valid_speaker_ids[0]
                    backfilled = 0
                    for sc in parsed.get("scenes") or []:
                        if not isinstance(sc, dict):
                            continue
                        for ln in sc.get("lines") or []:
                            if not isinstance(ln, dict):
                                continue
                            if not isinstance(ln.get("speaker"), str):
                                ln["speaker"] = single
                                backfilled += 1
                    logger.info(
                        "[speaker_drift] speaker_profiles=%s だが全 line "
                        "speaker 未設定 → '%s' を %d line に backfill",
                        valid_speaker_ids, single, backfilled,
                    )
                else:
                    logger.warning(
                        "[speaker_drift] speaker_profiles=%s (%d speakers) "
                        "だが全 line speaker 未設定。誰が何を話したかが "
                        "不明のため backfill 不可。Stage 1 UI で人間が "
                        "line ごとの speaker を選び直す必要あり",
                        valid_speaker_ids, len(valid_speaker_ids),
                    )

    # ── casting 提案の正規化 (= character_catalog 提供時のみ) ──
    # featured_characters / speaker_to_ref は best-effort 提案。catalog の
    # refs に無い ref は drop し、人間が Stage 1 UI で訂正する前提。
    # speaker_profiles は character_catalog の有無に関わらず素通し。
    if character_catalog:
        ...
        # speaker_to_ref: catalog refs に無い ref を drop
        raw_s2r = parsed.get("speaker_to_ref")
        cleaned_s2r: dict[str, str] = {}
        if isinstance(raw_s2r, dict):
            for spk, ref in raw_s2r.items():
                if (
                    isinstance(spk, str)
                    and isinstance(ref, str)
                    and ref in valid_refs
                ):
                    cleaned_s2r[spk] = ref
            dropped = len(raw_s2r) - len(cleaned_s2r)
            if dropped:
                logger.info(
                    "[casting] speaker_to_ref: %d 件の未知 ref を drop", dropped,
                )

        # Rule B: distinct character — 同 base を複数 speaker に割り当てない
        # (= 2 件目以降を drop。人間が後で対応付ける)。
        if cleaned_s2r:
            seen_bases: set[str] = set()
            deduped_s2r: dict[str, str] = {}
            for spk, ref in cleaned_s2r.items():
                base = ref.split("__", 1)[0]
                if base in seen_bases:
                    logger.info(
                        "[casting] speaker_to_ref: '%s' → '%s' を base 重複で "
                        "drop (distinct character rule)", spk, ref,
                    )
                    continue
                seen_bases.add(base)
                deduped_s2r[spk] = ref
            cleaned_s2r = deduped_s2r

        # ── 共通計算: speaker_to_locs / loc_to_wardrobes / base_to_refs ──
        # Rule A (wardrobe swap) と Phase D (unmapped fallback) で再利用する。
        loc_to_wardrobes = _compute_loc_to_wardrobes(location_catalog or [])
        base_to_refs = _compute_base_to_refs(character_catalog)
        speaker_to_locs = _compute_speaker_to_locs(parsed)

        # Rule A: wardrobe-by-location swap — 各 speaker の dominant location
        # の recommended_wardrobes に wardrobe を合わせる。location_catalog
        # 不在 / recommended_wardrobes 空 / 適合 wardrobe バリアント無し のいずれ
        # かなら何もしない (= Claude の選択を尊重、graceful)。
        if cleaned_s2r and location_catalog:
            swapped_s2r: dict[str, str] = {}
            for spk, ref in cleaned_s2r.items():
                base, _, wardrobe = ref.partition("__")
                dominant = _dominant_location(spk, speaker_to_locs)
                if dominant is None:
                    swapped_s2r[spk] = ref
                    continue
                recs = loc_to_wardrobes.get(dominant) or []
                if not recs or wardrobe in recs:
                    swapped_s2r[spk] = ref
                    continue
                char_refs = base_to_refs.get(base) or []
                candidate: str | None = None
                for w in recs:
                    cand = f"{base}__{w}"
                    if cand in char_refs:
                        candidate = cand
                        break
                if candidate and candidate != ref:
                    logger.info(
                        "[casting] %s: wardrobe を location '%s' の "
                        "recommended_wardrobes に合わせて '%s' → '%s' に swap",
                        spk, dominant, ref, candidate,
                    )
                    swapped_s2r[spk] = candidate
                else:
                    swapped_s2r[spk] = ref
            cleaned_s2r = swapped_s2r

        # Phase D: deterministic fallback — Claude が埋め残した speaker を
        # speaker_profiles の gender/age と catalog の appearance で matching し、
        # base_to_refs から決定論的に補完する。これで speaker_profiles を持つ
        # 全 speaker が必ず 1 つ ref を持つ状態に揃う (= UI 側の「未選択」状態が
        # 構造的に発生しなくなる)。precise でない場合も人間が Stage 1 UI で
        # 訂正できる前提。
        speaker_profiles = parsed.get("speaker_profiles") or {}
        if isinstance(speaker_profiles, dict) and speaker_profiles:
            cleaned_s2r = _fill_unmapped_speakers(
                speaker_profiles=speaker_profiles,
                cleaned_s2r=cleaned_s2r,
        ...
        if cleaned_s2r:
            parsed["speaker_to_ref"] = cleaned_s2r
        else:
            parsed.pop("speaker_to_ref", None)

        # featured_characters: cleaned_s2r 由来の ref を優先し、それ以外を
        # raw_feat から拾う。同 base の重複は禁止 (= cleaned_s2r が確定済み)。
        s2r_bases: set[str] = set()
        final_feat: list[str] = []
        for ref in cleaned_s2r.values():
            base = ref.split("__", 1)[0]
            s2r_bases.add(base)
            if ref not in final_feat:
                final_feat.append(ref)
        raw_feat = parsed.get("featured_characters")
        if isinstance(raw_feat, list):
            for ref in raw_feat:
                if not (isinstance(ref, str) and ref in valid_refs):
                    continue
                base = ref.split("__", 1)[0]
                if base in s2r_bases:
                    continue  # speaker_to_ref で確定済みの base はスキップ
                if ref not in final_feat:
                    final_feat.append(ref)
        if final_feat:
