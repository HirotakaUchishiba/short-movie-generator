#!/usr/bin/env python3
"""Stage 4 (Kling 動画) のグローバルキャッシュを LRU で削除する CLI。

容量上限を指定し、超過分を ``last_used_at`` の古い順に削除する。

例:
    python3 scripts/prune_kling_cache.py --max-bytes 1T
    python3 scripts/prune_kling_cache.py --max-bytes 500G --dry-run

cron 推奨運用 (毎日 4:00 に 2TB 制限):
    0 4 * * *  cd /path/to/repo && python3 scripts/prune_kling_cache.py --max-bytes 2T >> logs/prune.log 2>&1
"""
...
def main() -> int:
    ...
    print(f"cache root: {kling_cache._cache_root()}")
    ...
    res = kling_cache.prune(max_total_bytes=args.max_bytes,
