...
"""screenplays/*.json と screenplays/drafts/*.json から下記フィールドを除去する。
  ...
  - scenes[]: time, animation_prompt_auto
  - scenes[].lines[]: speaker, pause_before, breath_before
  - scenes[].lines[].acoustic: pitch_hz_mean

使い方:
    python3 scripts/migrate_screenplay_v2.py            # dry-run
    python3 scripts/migrate_screenplay_v2.py --apply    # 上書き保存
"""
import argparse
import json
import sys
...
def _strip(screenplay: dict) -> list[str]:
    ...
    for s_idx, scene in enumerate(screenplay.get("scenes") or []):
        ...
        for l_idx, line in enumerate(scene.get("lines") or []):
            ...
            ac = line.get("acoustic")
            if isinstance(ac, dict):
                for k in ACOUSTIC_DROP:
                    if k in ac:
                        ac.pop(k)
                        removed.append(f"scenes[{s_idx}].lines[{l_idx}].acoustic.{k}")
    return removed
...
def _iter_targets() -> list[Path]:
    ...
    for d in (ROOT / "screenplays", ROOT / "screenplays" / "drafts"):
        if d.is_dir():
            targets.extend(sorted(p for p in d.glob("*.json") if p.is_file()))
    return targets
