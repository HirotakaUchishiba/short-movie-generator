...
- 単一人物の動画 (= speaker タグ無し) では speaker_profiles を完全に省略する

入力 user content に "# 利用可能な character 集合" セクションが含まれている場合、加えて casting を提案する:
- speaker_to_ref: speaker_N → resolved id のマッピング。各 speaker の speaker_profiles を catalog の appearance と突合し、最も近いキャラを選ぶ。appearance が無いキャラも候補に含めてよい (= id や description から妥当に推測)
...
- **speaker_profiles を持つ全 speaker に対して speaker_to_ref を必ず 1 つ提案する** (= 確信度が低くても best guess を選ぶ。空欄は許可しない)。提案精度が低い場合も人間が Stage 1 UI で訂正できるので、まず埋めることを優先する。catalog 枯渇等で本当に提案不能なときのみ post-process が deterministic fallback で補完する
character 集合が提供されていない場合は featured_characters / speaker_to_ref を完全に省略する (= 旧挙動互換)。speaker_profiles は character 集合の有無に関わらず出力する。
...
def _appearance_match_score(
    profile: dict, appearance: dict
) -> int:
    """speaker_profile と character.appearance のマッチング度を 0-3 で返す。

    - +2: gender が両方あり一致 (mismatch なら -100 で hard reject)
    - +1: age_range が両方あり一致 (mismatch でも 0 = soft)
    - +0: 片方が空 (= unknown は中立)
    """

    score = 0
    p_gender = (profile.get("gender") or "").strip().lower()
    a_gender = (appearance.get("gender") or "").strip().lower()
    if p_gender and a_gender:
        if p_gender == a_gender:
            score += 2
        else:
            return -100  # hard reject — 性別不一致は除外
    p_age = (profile.get("age_range") or "").strip().lower()
    a_age = (appearance.get("age_range") or "").strip().lower()
    if p_age and a_age and p_age == a_age:
        score += 1
    return score
...
def _fill_unmapped_speakers(
    *,
    ...
    アルゴリズム:
      ...
      3. 候補 base を appearance match score で sort (高いほど優先)
      4. 同 score の中では base_id alphabetical
      5. **distinct character rule**: 既使用 base を除外
      6. 候補枯渇 (= 全 base が使用済み) なら distinct rule を緩めて再利用
      7. 選ばれた base から wardrobe-aware ref を取り、cleaned_s2r に追加
    """
    ...
    # 全 base + appearance を 1 度だけ抽出 (= catalog 巡回コスト削減)
    bases_with_appearance: list[tuple[str, dict]] = []
    for entry in character_catalog:
        if not isinstance(entry, dict):
            continue
        base_id = entry.get("id")
        ...
        if not isinstance(appearance, dict):
            appearance = {}
        bases_with_appearance.append((base_id, appearance))

    for spk in unmapped:
        profile = speaker_profiles.get(spk) or {}
        if not isinstance(profile, dict):
            profile = {}
        # score 計算 + sort
        scored: list[tuple[int, str]] = []
        for base_id, appearance in bases_with_appearance:
            score = _appearance_match_score(profile, appearance)
            if score >= 0:  # hard reject 以外を残す
                scored.append((score, base_id))
        # 降順 score / 昇順 base_id (= deterministic)
        scored.sort(key=lambda x: (-x[0], x[1]))

        chosen_base: str | None = None
        # Step A: distinct rule で空き base を探す
        for _score, base_id in scored:
            if base_id not in used_bases:
                chosen_base = base_id
                break
        # Step B: 枯渇したら distinct rule を緩めて再利用 (= 全部使い切ったら
        # gender 一致の中で最初の候補を再利用、それすら無ければ catalog 先頭)
        if chosen_base is None:
            for _score, base_id in scored:
                chosen_base = base_id
                break
        # Step C: scored 空 (= 全部 gender hard reject) なら gender 無視で
        # base_to_refs から先頭を採用 (= 完全 fallback、人間に編集を委ねる)
        if chosen_base is None:
            sorted_bases = sorted(base_to_refs)
            if sorted_bases:
                chosen_base = sorted_bases[0]
        if chosen_base is None:
            # base_to_refs も空 = catalog が空。fill 不能なので skip
            continue
        ...
        if ref is None:
            continue
        logger.info(
            "[casting] %s: unmapped speaker を fallback で '%s' に割当 "
            "(profile=%s)", spk, ref, profile,
        )
        out[spk] = ref
