...
def build_screenplay(
    *,
    ...
    if character_catalog:
        ...
        # featured_characters: catalog refs に無い ref を drop し、
        # speaker_to_ref の値との和集合 (= 順序維持・重複排除) にする
        cleaned_feat: list[str] = []
        ...
        if isinstance(raw_feat, list):
            for ref in raw_feat:
                if (
                    isinstance(ref, str)
                    and ref in valid_refs
                    and ref not in cleaned_feat
                ):
                    cleaned_feat.append(ref)
        for ref in cleaned_s2r.values():
            if ref not in cleaned_feat:
                cleaned_feat.append(ref)
        if cleaned_feat:
            parsed["featured_characters"] = cleaned_feat
        else:
