...
本ドキュメントは「cron で参考動画を取得 → 抽象台本生成 → 本パイプラインを完全自動実行 → SNS 公開 → メトリクス取得 → 次回生成へフィードバック」という閉ループを **品質を毀損せずに** 完成させるための綿密な実装計画を残す。判定根拠と全体俯瞰は `docs/plannings/2026-05-07_full-automation-feasibility.md` を参照。
